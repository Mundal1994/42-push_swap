...
static void	push_and_update_helper(t_stack *stack, char c, int d)
{
	...
	else
	{
		...
		if (d == FALSE && stack->b_empty == FALSE && stack->a[stack->top_a + 1] < stack->b_big)
			while (stack->b[stack->top_b] > stack->a[stack->top_a + 1])
				solve_and_print(stack, "rb");
		else if (stack->b_empty == FALSE && stack->a[stack->top_a + 1] < stack->b_big)
			while (stack->b[stack->bottom - 1] < stack->a[stack->top_a + 1])
